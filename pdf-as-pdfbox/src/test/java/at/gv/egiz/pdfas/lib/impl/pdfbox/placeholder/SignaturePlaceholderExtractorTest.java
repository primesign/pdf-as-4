...
public class SignaturePlaceholderExtractorTest {
	...
	/**
	 * Tests extraction of all placeholders of a certain file. The candidate file contains five placeholders with
	 * identifier "PDF-AS-POS" and two non-placeholder images, for false-positive testing purposes. The tests expects
	 * five placeholders with certain positions and names.
	 */
	@Test
	public void testExtractAllPlaceholders() throws PdfAsException, IOException {
		...
		try (InputStream in = getClass().getResourceAsStream(CANDIDATE_RESOURCE_URI)) {
			...
			for (SignaturePlaceholderData placeholder : placeholders) {
				...
				System.out.println("Found placeholder:  p:" + t.getPage() + ";x:" + t.getPosX() + ";y:" + t.getPosY()
						+ ";w:" + t.getWidth() + ", based on image '" + placeholder.getPlaceholderName() + "'.");
			}

			assertEquals("Image7", placeholders.get(0).getPlaceholderName());
			assertEquals(0, CompareToBuilder.reflectionCompare(new TablePos("p:1;x:70.8;y:513.95996;w:452.76"), placeholders.get(0).getTablePos()));

			assertEquals("Image13", placeholders.get(1).getPlaceholderName());
			assertEquals(0, CompareToBuilder.reflectionCompare(new TablePos("p:3;x:70.8;y:287.76;w:452.76"), placeholders.get(1).getTablePos()));

			assertEquals("Image16", placeholders.get(2).getPlaceholderName());
			assertEquals(0, CompareToBuilder.reflectionCompare(new TablePos("p:4;x:70.8;y:272.4;w:452.76"), placeholders.get(2).getTablePos()));

			assertEquals("Image19", placeholders.get(3).getPlaceholderName());
			assertEquals(0, CompareToBuilder.reflectionCompare(new TablePos("p:5;x:70.8;y:544.92004;w:452.76"), placeholders.get(3).getTablePos()));

			assertEquals("Image20", placeholders.get(4).getPlaceholderName());
			assertEquals(0, CompareToBuilder.reflectionCompare(new TablePos("p:5;x:70.8;y:305.16;w:453.84"), placeholders.get(4).getTablePos()));
